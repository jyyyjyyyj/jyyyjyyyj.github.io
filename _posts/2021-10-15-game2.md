...
![](../assets/games2.png)
<br>
我设置的是当主角靠近小猫弹出对话框之后，按下手柄的"X"键让它自动跟随主角。首先我们需要检测出当对话框弹出的时候，用`if(Input.GetButton("button_name"))`函数来检测"X"键是否被按下。其中""里的内容是自定义的按键名称，可以在edit -> project settings -> input里设置。一般情况下，手柄"X"键对应的键值是"joystick button 2"，网上很容易能查到手柄的每个键的键值。
