---
title: Unity 2D小游戏开发(1) - NPC跟随主角走动
subtitle: Develop a 2D game on Unity(1) - let NPC follow the hero
tags: [unity]
private: true
---
<style>
...
我设置的是当主角靠近小猫弹出对话框之后，按下手柄的"X"键让它自动跟随主角。首先我们需要检测出当对话框弹出的时候，用`if(Input.GetButton("Choose"))`函数来检测 "X"键是否被按下。其中""里的内容是自定义的键值，可以在edit -> project settings -> input里自行设置。一般情况下，手柄"X"键的名称是"joystick button 2"，可以查自己手柄型号所对应的键值。
