...
![enter description here](./images/pear3_1.png)

这样一来，就可以写出判断鼠标是否选中控制点的流程了：

1. 判断鼠标左键是否按下，如果按下，获取鼠标的屏幕坐标。

2. 将屏幕坐标通过反投影转换成三维的世界坐标，此时代入0和1两个不同的z，获取鼠标所产生的线段的起始和终点。
...
![enter description here](./images/coord.png)
...
void onmouse(int button, int state, int x, int y)
{
	...
	case GLUT_LEFT_BUTTON:
		if (state == GLUT_DOWN && drag == false)
		{
			...
			//获取被选中的控制点坐标
			//通过向量叉乘来计算控制点到线段的距离，如果距离小于阈值，该点被选中，如果没有选中控制点，返回-1
		...
		else
		{
			...
			if (drag)
			{
				...
				gluUnProject((GLdouble)newx, (GLdouble)newy, (GLdouble)newz, mvmatrix, projmatrix, viewport, &wx1, &wy1, &wz1);
